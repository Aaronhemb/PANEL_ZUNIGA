...
if (isset($_POST['signup'])) {
	...
	$cpassword = base64_encode($_POST["password"]);
	...
	if (!$error) {
		if(mysqli_query($con, "INSERT INTO usuarios(unique_id,nombre,email,password,fecha_crea)
		VALUES('" . $ran_id . "','" . $nombre . "', '" . $email . "', '" . $salt . "'  , '" . $mifecha . "')")) {
			...
			<form role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" name="signupform"  enctype="multipart/form-data"   >
				<fieldset>
					...
					<div class="form-group">
						...
						<input type="text" name="nombre" placeholder="Nombre(s)" required value="<?php if($error) echo $nombre; ?>" class="form-control" />
